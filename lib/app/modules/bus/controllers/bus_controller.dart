...
class BusController extends GetxController {
  ...
  Future<void> getDeparture() async {
    try {
      ...
      QuerySnapshot departureQuery =
          await FirebaseFirestore.instance.collection('Departures').get();
    ...
    try {
      ...
      if (DateFormat("yyyy-MM-dd").format(selectedDate) ==
        ...
        _departureList = departureList
            .where((element) =>
                element.routes.departureStation.stationId ==
                    departureStationId &&
                element.routes.arrivalStation.stationId == arrivalStationId &&
                timeFormat
                    .parse(timeFormat.format(element.routes.departureTime))
                    .isAfter(timeFormat.parse(nowTimeStr)))
            .toList();
      } else {
        _departureList = departureList
            .where((element) =>
      ...
      for (int i = 0; i < _departureList.length; i++) {
        ...
        QuerySnapshot capacityQuerySnapshot = await FirebaseFirestore.instance
            .collection('Booking')
            .where('departure_id', isEqualTo: _departureList[i].departureId)
            .where('book_date', isGreaterThanOrEqualTo: startDate)
            .where('book_date', isLessThan: endDate)
            .where("ticket_id",
                isEqualTo: _departureList[i].buses.tickets.first.ticketId)
            .get();

        QuerySnapshot capacityVipQuerySnapshot = await FirebaseFirestore
            .instance
            .collection('Booking')
            .where('departure_id', isEqualTo: _departureList[i].departureId)
            .where('book_date', isGreaterThanOrEqualTo: startDate)
            .where('book_date', isLessThan: endDate)
            .where("ticket_id",
                isEqualTo: _departureList[i].buses.tickets.last.ticketId)
            .get();

        int countCapacity = _departureList[i].buses.capacity <= 0
            ? 10000
            : capacityQuerySnapshot.size;
        int countCapacityVip = _departureList[i].buses.capacityVIP <= 0
            ? 10000
            : capacityVipQuerySnapshot.size;
        if (countCapacity >= _departureList[i].buses.capacity &&
            countCapacityVip >= _departureList[i].buses.capacityVIP) {
